...
class MvPDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src_views,
        ...
        # tgt_batch = []
        q = k = self.with_pos_embed(tgt, query_pos)   # 此处tgt为V，query_pos为K
        tgt2 = self.self_attn(q.transpose(0, 1),     # 自注意力融合
                              k.transpose(0, 1),
        ...
        return tgt      # 初步认为这就是融合完的结果，不过此时输出的尺寸可能是基于关节的，要是基于顶点可能需要更改一下参数
